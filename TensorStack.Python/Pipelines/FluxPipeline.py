...
_stopwatch = None
_pipelineMap = {
    ProcessType.TextToImage: FluxPipeline,
    ProcessType.ImageToImage: FluxImg2ImgPipeline,
    ProcessType.ImageEdit: FluxKontextPipeline,
    ProcessType.ImageInpaint: FluxInpaintPipeline,
    ProcessType.ImageControlNet: FluxControlNetPipeline,
}
...
def load_text_encoder(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    if single_path and single_path.is_file():
        ...
        text_encoder = Utils.from_component(FluxPipeline, "text_encoder", single_path, template_path, _device_map, config.data_type)
        if text_encoder:
...
def load_vae(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    if single_path and single_path.is_file():
        ...
        auto_encoder = Utils.from_component(FluxPipeline, "vae", single_path, template_path, _device_map, config.data_type)
        if auto_encoder:
            Utils.trim_memory(True)
            return auto_encoder
...
def load_control_net(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    _control_net_cache = FluxControlNetModel.from_pretrained(
...
def generate(
        ...
        with torch.no_grad():
            ...
            if options.guidance_scale > 1:
                negative_prompt_value = _pipeline.encode_prompt(
                    ...
                    prompt_2=options.negative_prompt,
                    device=_pipeline._execution_device,
                    max_sequence_length=512
                )
    ...
    (prompt_embeds, pooled_prompt_embeds, text_ids) = prompt_cache
    (negative_prompt_embeds, negative_pooled_prompt_embeds, text_ids) = negative_prompt_cache
    pipeline_options = {
    ...
    if _processType in (ProcessType.ImageToImage, ProcessType.ImageEdit):
