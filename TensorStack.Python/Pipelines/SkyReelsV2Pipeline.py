...
_stopwatch = None
_pipelineMap = {
    ProcessType.TextToVideo: SkyReelsV2DiffusionForcingPipeline,
    ProcessType.ImageToVideo: SkyReelsV2DiffusionForcingImageToVideoPipeline,
    ProcessType.VideoToVideo: SkyReelsV2DiffusionForcingVideoToVideoPipeline
}
...
#------------------------------------------------
# Load SkyReelsV2Transformer3DModel
#------------------------------------------------
def load_transformer(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    if transformer_path.is_file():
        ...
        transformer =  SkyReelsV2Transformer3DModel.from_single_file(
...
def load_vae(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    if single_path and single_path.is_file():
        ...
        auto_encoder = Utils.from_component(SkyReelsV2DiffusionForcingPipeline, "vae", single_path, template_path, _device_map, torch.float32)
        if auto_encoder:
            Utils.trim_memory(True)
            return auto_encoder
...
def load_control_net(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    # _control_net_cache = ontrolNetModel.from_pretrained(
...
def generate(
    ...
    pipeline_options = {
        ...
        "base_num_frames": options.frame_chunk,
        "overlap_history": options.frame_chunk_overlap,
        "addnoise_condition": options.noise_condition,
