...
def create_pipeline(config: DataObjects.PipelineConfig):
    global _progress_tracker, _is_video_pipeline
    _progress_tracker = Utils.ModelDownloadProgress(total_models=4)
    if _processType in ("TextToVideo", "ImageToVideo"):
    ...
    vae = (
        load_vae_video(config, pipeline_config, quant_config_diffusers, pipeline_kwargs) 
        if _is_video_pipeline 
        else load_vae_image(config, pipeline_config, quant_config_diffusers, pipeline_kwargs)
    )
