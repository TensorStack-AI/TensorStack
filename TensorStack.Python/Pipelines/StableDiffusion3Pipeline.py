...
# Load T5Tokenizer
#------------------------------------------------
def load_tokenizer_3(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    if _pipeline and _pipeline.tokenizer_3:
        print(f"[Load] Loading Cached Tokenizer3")
        return _pipeline.tokenizer_3

    tokenizer_3_path: Path = _model_config["tokenizer_3"]
    tokenizer_3_config: Path = _model_config["tokenizer_3_config"]

    # 1. Load from pretrained folder
    print(f"[Load] Loading Pretrained Tokenizer3")
    tokenizer_3 = T5Tokenizer.from_pretrained(
        tokenizer_3_path,
        config=tokenizer_3_config,
        dtype=config.data_type,
        **pipeline_kwargs
    )
    return tokenizer_3
...
def load_text_encoder(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    if single_path and single_path.is_file():
        ...
        text_encoder = Utils.from_component(StableDiffusion3Pipeline, "text_encoder", single_path, template_path, _device_map, config.data_type)
        if text_encoder:
...
def load_text_encoder_2(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    if single_path and single_path.is_file():
        ...
        text_encoder_2 = Utils.from_component(StableDiffusion3Pipeline, "text_encoder_2", single_path, template_path, _device_map, config.data_type)
        if text_encoder_2:
            Utils.trim_memory(True)
...
def load_text_encoder_3(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    if _pipeline and _pipeline.text_encoder_3:
        print(f"[Load] Loading Cached TextEncoder3")
        return _pipeline.text_encoder_3

    text_encoder_3_path: Path = _model_config["text_encoder_3"]
    text_encoder_3_config: Path = _model_config["text_encoder_3_config"]

    # 1. Load from pretrained folder
    print(f"[Load] Loading Pretrained TextEncoder2")
    text_encoder_3 = T5EncoderModel.from_pretrained(
        text_encoder_3_path,
        config=text_encoder_3_config,
        dtype=config.data_type,
...
def load_vae(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    if single_path and single_path.is_file():
        ...
        auto_encoder = Utils.from_component(StableDiffusion3Pipeline, "vae", single_path, template_path, _device_map, config.data_type)
        if auto_encoder:
            Utils.trim_memory(True)
            return auto_encoder
...
def create_pipeline(config: DataObjects.PipelineConfig):
    ...
    tokenizer_3 = load_tokenizer_3(config, pipeline_kwargs)
