...
def create_pipeline(config: DataObjects.PipelineConfig, download_only: bool = False):
    ...
    text_encoder_3 = load_text_encoder_3(config, pipeline_kwargs, download_only)
...
def load_text_encoder_3(
    ...
    if checkpoint_config.text_encoder_checkpoint is not None:
        print(f"[Load] Loading checkpoint TextEncoder3")
    ...
    print(f"[Load] Loading TextEncoder3")
...
def load_vae(
    ...
    if checkpoint:
        ...
        auto_encoder = Utils.load_component(
            StableDiffusion3Pipeline, 
            config.base_model_path, 
            checkpoint, 
            "vae", 
