...
def load_transformer_2(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    checkpoint = (
        config.checkpoint_config.transformer_2
        if config.checkpoint_config.transformer_2
        else config.checkpoint_config.single_file
    )
    if checkpoint:
        print(f"[Load] Loading Checkpoint Transformer2, IsOffline: {config.is_offline_mode}")
        is_gguf = Utils.isGGUF(checkpoint)
        transformer = WanTransformer3DModel.from_single_file(
            checkpoint,
    ...
    print(f"[Load] Loading Pretrained Transformer2, IsOffline: {config.is_offline_mode}")
    transformer = WanTransformer3DModel.from_pretrained(
        config.base_model_path,
