...
_stopwatch = None
_pipelineMap = {
    ProcessType.TextToImage: GlmImagePipeline,
    ProcessType.ImageEdit: GlmImagePipeline
}
...
#------------------------------------------------
# Load GlmImageForConditionalGeneration
#------------------------------------------------
def load_vision_language_encoder(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    if _pipeline and _pipeline.vision_language_encoder:
        print(f"[Load] Loading Cached VisionLanguageEncoder")
        return _pipeline.vision_language_encoder

    vision_language_encoder_path: Path = _model_config["text_encoder_2"]
    vision_language_encoder_config: Path = _model_config["text_encoder_2_config"]
...
#------------------------------------------------
# Load GlmImageTransformer2DModel
#------------------------------------------------
...
def load_vae(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    if single_path and single_path.is_file():
        ...
        auto_encoder = Utils.from_component(GlmImagePipeline, "vae", single_path, template_path, _device_map, config.data_type)
        if auto_encoder:
            Utils.trim_memory(True)
            return auto_encoder
...
def create_pipeline(config: DataObjects.PipelineConfig):
    ...
    vision_language_encoder = load_vision_language_encoder(config, pipeline_kwargs)
