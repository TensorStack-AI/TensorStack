...
_stopwatch = None
_pipelineMap = {
    ProcessType.TextToVideo: CogVideoXPipeline,
    ProcessType.ImageToVideo: CogVideoXImageToVideoPipeline,
    ProcessType.VideoToVideo: CogVideoXVideoToVideoPipeline,
}
...
def load_vae(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    if single_path and single_path.is_file():
        ...
        auto_encoder = Utils.from_component(CogVideoXPipeline, "vae", single_path, template_path, _device_map, config.data_type)
        if auto_encoder:
            Utils.trim_memory(True)
            return auto_encoder
...
def generate(
    ...
    if _processType == ProcessType.ImageToVideo:
        pipeline_options.update({ "image": images, "use_dynamic_cfg": True })
