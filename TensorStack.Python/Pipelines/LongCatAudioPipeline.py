...
_stopwatch = None
_pipelineMap = {
    ProcessType.TextToAudio: LongCatAudioDiTPipeline,
}
...
#------------------------------------------------
# Load TextEncoder UMT5EncoderModel
#------------------------------------------------
...
#------------------------------------------------
# Load LongCatAudioDiTTransformer
#------------------------------------------------
def load_transformer(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    if transformer_path.is_file():
        ...
        transformer =  LongCatAudioDiTTransformer.from_single_file(
    ...
    transformer =  LongCatAudioDiTTransformer.from_pretrained(
        str(transformer_path),
        torch_dtype=config.data_type,
        device_map=_device_map,
...
def load_vae(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    if vae_path.is_file():
        ...
        auto_encoder =  LongCatAudioDiTVae.from_single_file(
            str(vae_path),
    ...
    if single_path and single_path.is_file():
        ...
        auto_encoder = Utils.from_component(LongCatAudioDiTPipeline, "vae", single_path, template_path, _device_map, config.data_type)
        if auto_encoder:
            Utils.trim_memory(True)
            return auto_encoder
    ...
    auto_encoder = LongCatAudioDiTVae.from_pretrained(
        str(vae_path),
        torch_dtype=config.data_type,
        device_map=_device_map,
        **pipeline_kwargs
...
def generate(
    ...
    pipeline_options = {
        ...
        "audio_duration_s": None if options.duration == 0 else options.duration,
    ...
    audio = output.squeeze()  # (channels, samples), 424 kHz
    sf.write(options.temp_filename, audio, _pipeline.sample_rate)
