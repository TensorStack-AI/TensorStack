...
_stopwatch = None
_pipelineMap = {
    ProcessType.TextToImage: QwenImagePipeline,
    ProcessType.ImageToImage: QwenImageImg2ImgPipeline,
    ProcessType.ImageInpaint: QwenImageInpaintPipeline,
    ProcessType.ImageEdit: QwenImageEditPlusPipeline,
    ProcessType.ImageControlNet: QwenImageControlNetPipeline,
}
...
#------------------------------------------------
# Load QwenImageTransformer2DModel
#------------------------------------------------
...
def load_vae(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    if single_path and single_path.is_file():
        ...
        auto_encoder = Utils.from_component(QwenImagePipeline, "vae", single_path, template_path, _device_map, config.data_type)
        if auto_encoder:
            Utils.trim_memory(True)
            return auto_encoder
...
def load_control_net(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    _control_net_cache = QwenImageControlNetModel.from_pretrained(
...
def generate(
    ...
    (prompt_embeds, prompt_embeds_mask) = prompt_cache
    (negative_prompt_embeds, negative_prompt_embeds_mask) = negative_prompt_cache
    pipeline_options = {
