...
from transformers import T5Tokenizer, Qwen2Tokenizer, Qwen3Model
...
_stopwatch = None
_pipelineMap = {
    ProcessType.TextToImage: AnimaPipeline,
}
...
def load_tokenizer_2(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    if _pipeline and _pipeline.t5_tokenizer:
        print(f"[Load] Loading Cached Tokenizer2")
        return _pipeline.t5_tokenizer

    tokenizer_2_path: Path = _model_config["condition_encoder"]
    tokenizer_2_config: Path = _model_config["condition_encoder_config"]
...
#------------------------------------------------
# Load AnimaTextConditioner
#------------------------------------------------
def load_text_conditioner(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    if _pipeline and _pipeline.text_conditioner:
        print(f"[Load] Loading Cached TextConditioner")
        return _pipeline.text_conditioner
    ...
    # 1. Load component from single file
    if single_path and single_path.is_file():
        print(f"[Load] Loading Component TextConditioner")
        text_conditioner = Utils.from_component(AnimaPipeline, "text_conditioner", single_path, template_path, _device_map, config.data_type)
        if text_conditioner:
            Utils.trim_memory(True)
            return text_conditioner

    # 2. Load from pretrained folder
    print(f"[Load] Loading Pretrained TextConditioner")
...
def load_vae(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    if vae_path.is_file():
        ...
        auto_encoder =  AutoencoderKLQwenImage.from_single_file(
            str(vae_path),
    ...
    if single_path and single_path.is_file():
        ...
        auto_encoder = Utils.from_component(AnimaPipeline, "vae", single_path, template_path, _device_map, config.data_type)
        if auto_encoder:
            Utils.trim_memory(True)
            return auto_encoder
    ...
    auto_encoder = AutoencoderKLQwenImage.from_pretrained(
        str(vae_path),
        torch_dtype=config.data_type,
        device_map=_device_map,
        **pipeline_kwargs
...
def create_pipeline(config: DataObjects.PipelineConfig):
    ...
    text_conditioner = load_text_conditioner(config, pipeline_kwargs)
