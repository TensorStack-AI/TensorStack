...
_stopwatch = None
_pipelineMap = {
    ProcessType.TextToVideo: HeliosPyramidPipeline,
    ProcessType.ImageToVideo: HeliosPyramidPipeline,
    ProcessType.VideoToVideo: HeliosPyramidPipeline
}
...
def load_vae(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    if single_path and single_path.is_file():
        ...
        auto_encoder = Utils.from_component(HeliosPyramidPipeline, "vae", single_path, template_path, _device_map, torch.float32)
        if auto_encoder:
            Utils.trim_memory(True)
            return auto_encoder
