    ...
    public sealed class KVCacheEncoderDecoder : IKVCache
    {
        ...
        public void Initialize(int initialSize)
        {
            ...
            for (var i = 0; i < _values.Length; ++i)
            {
                if (i % 4 == 0)
                {
                    _values[i] = OrtValue.CreateAllocatedTensorValue(OrtAllocator.DefaultInstance, _dataType, decoderDims);    // Decoder Key
                    _values[i + 1] = OrtValue.CreateAllocatedTensorValue(OrtAllocator.DefaultInstance, _dataType, decoderDims);// Decoder Val
                    _values[i + 2] = OrtValue.CreateAllocatedTensorValue(OrtAllocator.DefaultInstance, _dataType, encoderDims);// Encoder Key
                    _values[i + 3] = OrtValue.CreateAllocatedTensorValue(OrtAllocator.DefaultInstance, _dataType, encoderDims);// Encoder Val
                }
            }
        }
